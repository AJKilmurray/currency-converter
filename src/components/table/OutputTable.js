				...
				<TableBody>
					{results.map((result) => (
						<TableRow key={result.key}>
							<TableCell component="th" scope="row" className="conversion-title">
								{result.key.toUpperCase()}
							</TableCell>
							<TableCell align="right" className="conversion">
								<div className="flex-container">
									{result.value.toFixed(2)} {result.key.toUpperCase()} <ArrowForwardIcon color="error" />
									{amount.value > 0 ? amount.value : 1} {currency.value.toUpperCase()}{" "}
								</div>
							</TableCell>
						</TableRow>
					))}
